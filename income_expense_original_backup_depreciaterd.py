    ...
    for i in tqdm(files):
        ...
        try:
            ...
            begindate = begindate_check.text if ET.iselement(begindate_check) else begindate_chcek2.text if \
                ET.iselement(begindate_chcek2) else None

            if begindate is None or begindate == '':
                print(f"Could not find begin date for file: {i}")
                continue
            ...
            if ET.iselement(GrantorEIN_check):
                ...
                print(f"No valid EIN found for file: {i}")
                continue
            ...
            profile_dict = (i, {"EIN": GrantorEIN,
                               "Buisnes name": businessname_ln1.upper(),
            ...
            print(f"XML parsing error in file {i}: {e}")
            continue
            
        except Exception as e:
            print(f"Unexpected error processing file {i}: {e}")
            continue
