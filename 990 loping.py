...
for i in data[Filings]:
    ...
    if i['EIN'] in [row[0] for row in check_list_rows]: 
        ...
        if ET.iselement(ScheduleI)==True:
            ...
            for recipient in RecipientTablelist:
                    ...
                    recipient_EIN = recipient_EIN_check.text if (recipient_EIN_check is not None and ET.iselement(recipient_EIN_check)) else ''

                    business_name_check = recipient.find('./*/{http://www.irs.gov/efile}BusinessNameLine1Txt')
                    business_name = business_name_check.text if (business_name_check is not None and ET.iselement(business_name_check)) else ''

                    address_element_check = recipient.find('.*/{http://www.irs.gov/efile}AddressLine1Txt')
                    address_element_line1 = address_element_check.text if (address_element_check is not None and ET.iselement(address_element_check)) else ''

                    address_element_line2_check=recipient.find('.*/{http://www.irs.gov/efile}AddressLine2Txt')
                    address_element_line2 = address_element_line2_check.text if (address_element_line2_check is not None and ET.iselement(address_element_line2_check)) else ''

                    address_element_city_check=recipient.find('.*/{http://www.irs.gov/efile}CityNm')
                    address_element_city = address_element_city_check.text if (address_element_city_check is not None and ET.iselement(address_element_city_check)) else ''

                    address_element_state_check=recipient.find('.*/{http://www.irs.gov/efile}StateAbbreviationCd')
                    address_element_state = address_element_state_check.text if (address_element_state_check is not None and ET.iselement(address_element_state_check)) else ''

                    address_element_zip_check = recipient.find('.*/{http://www.irs.gov/efile}ZIPCd')
                    address_element_zip = address_element_zip_check.text if (address_element_zip_check is not None and ET.iselement(address_element_zip_check)) else ''


                    amount_element_check = recipient.find('./{http://www.irs.gov/efile}CashGrantAmt')
                    amount_element = amount_element_check.text if (amount_element_check is not None and ET.iselement(amount_element_check)) else ''

                    status_element_check =recipient.find('./{http://www.irs.gov/efile}IRCSectionDesc')
                    status_element = status_element_check.text if (status_element_check is not None and ET.iselement(status_element_check)) else ''

                    purpose_element_check=recipient.find('./{http://www.irs.gov/efile}PurposeOfGrantTxt')
                    purpose_element = purpose_element_check.text if (purpose_element_check is not None and ET.iselement(purpose_element_check)) else ''

                    address = (address_element_line1 or '') + ' ' + (address_element_line2 or '') + ' ' + (address_element_city or '') + ' ' + (address_element_state or '') + ' ' + (address_element_zip or '')
